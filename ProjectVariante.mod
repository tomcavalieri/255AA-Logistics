...
maximize Revenue: sum {(i,j) in Link} Profit[i,j] * Flight[i,j] - sum {i in Cities} CostActivation[i]* Activation[i]- sum {('Berlino',j) in Link}Flight['Berlino',j] - sum {(j,'Berlino') in Link}Flight[j,'Berlino']; 

subject to ActivationConstr: sum {j in Cities} Activation[j] <= 5;
subject to ActivationFlights: sum {(i,j) in Link} Flight[i,j] <= 14;
